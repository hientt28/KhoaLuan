<table class="table table-striped table-hover" id="tblData">
    <thead>
    <tr>
        <th class="th_chk"><input type="checkbox" id="checkAll"></th>
        <th>{{ trans('appliances.name') }}</th>
        <th>{{ trans('appliances.status') }}</th>
        <th>{{ trans('appliances.electric_value') }}</th>
        <th>{{ trans('appliances.action') }}</th>
    </tr>
    </thead>
    <tbody>
        @foreach($apps as $app)
            <tr id="{{ $app['id'] }}">
                <td class="chk">
                    <input type="checkbox" class="case" value="{{ $app['id'] }}"/>
                </td>
                <td>{{ $app->name }}</td>
                <td>{!! fill_status($app->status) !!}</td>
                <td>{{ $app->electric_value }}</td>

                <td class="col-md-2 td_action">
                    <a href="{{ route('admin.appliances.edit', [$app->id]) }}" class="btn btn-link">
                        <i class="fa fa-pencil fa-fw"></i>
                        {{ trans('common.edit') }}
                    </a>
                </td>

                <td class="col-md-2 td_action">
                    {!! Form::open(['route' => ['admin.appliances.destroy', $app['id']], 'method' => 'DELETE']) !!}
                    {!! Form::button('<i class="fa fa-remove fa-fw"></i> ' . trans('common.delete'), [
                        'type' => 'submit',
                        'class' => 'btn btn-link',
                        'onclick' => "return confirm('" . trans('common.confirm_delete') . "')"
                    ]) !!}
                    {!! Form::close() !!}
                </td>
            </tr>
        @endforeach
    </tbody>
</table>

<div id="pagination" class="pull-right">
    @if(count($apps) > 0)
        {{ $apps->render() }}
    @endif
</div>